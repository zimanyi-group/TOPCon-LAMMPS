...
ATOMS=re.compile('ITEM: ATOMS id type xs ys zs')
...
DumpCoords = re.compile(r'\s*\d+\s+\d+\s+\d+\.\d+\s+\d+\.\d+\s+\d+\.\d+')
...
class Simulation():
    ...
    def Update(self, dir):
        ...
        if(dir == 'next'):
            ...
            _start = self.start - 2*self.dl
        self.done = False
        #print(_start)
        #print("############\n")
        for i in range(_start,_start + self.dl + 1):
            if(TIMESTEP.match(self.m_lines[i])):
                self.set_Timestep(self.m_lines[i + 1])
            elif(DumpCoords.match(self.m_lines[i])):
                _id, _type, _x, _y, _z = self.m_lines[i].split()
                self.atoms[int(_id) -1].coords = np.array([float(_x)*s[0],float(_y)*s[1],float(_z)*s[2]], dtype = float)
        self.start = _start + self.dl
        return
...
def set_vals(inst,lines,i):
    if(NUM.match(lines[i])):
        inst.set_Num(lines[i+1])
    elif(BB.match(lines[i])):
        inst.set_BB_Dump(lines[i+1:i+4])
    elif(ATOMS.match(lines[i])):
        lns=lines[i+1:i+1+inst.num]
        inst.set_Atoms(lns, scaled = True)
    elif(TIMESTEP.match(lines[i])):
        inst.set_Timestep(lines[i+1])
    else:
        print("????")
        print(lines)
...
def Write_Data_WQ(file_name,Ens):
    ...
    Num = "{0:.0f} atoms\n".format(Ens.num)
    Typs = "3 atom types\n"
    Atoms = "{0:.0f} {1:.0f} {2:.6f} {3:.6f} {4:.6f} {5:.6f} 0 0 0\n"
    Vel = "{0:.0f} 0 0 0\n"
    Out.write(Head)
    Out.write("\n")
    Out.write(Num)
